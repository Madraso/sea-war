...
bool check_place(struct player *player){									
	bool still;
	for(int i=9;i>=player->current_ship;i--){
		
	int l=player->boat[i].length;
	int b=player->boat[player->current_ship].length;	
		for(int j=-1;j<=l;j++){
			if(((player->boat[player->current_ship].start_y!=player->boat[i].start_y)&&	
			(player->boat[player->current_ship].start_x+h!=player->boat[i].start_x+j))||
			((player->boat[player->current_ship].start_y!=player->boat[i].start_y-1)&&	
			(player->boat[player->current_ship].start_x+h!=player->boat[i].start_x+j))||
			((player->boat[player->current_ship].start_y!=player->boat[i].start_y+1)&&	
			(player->boat[player->current_ship].start_x+h!=player->boat[i].start_x+j)))||
			(((player->boat[player->current_ship].start_x!=player->boat[i].start_x)&&	
			(player->boat[player->current_ship].start_y+h!=player->boat[i].start_y+j))||
